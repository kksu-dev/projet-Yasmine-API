...
const getAllUser = async (req,res) => {
  try {
    ...
    res.status(500).json({ status:false,message: 'Une erreur est survenue lors de la récupération des utilisateurs.' });
  }
}
  ...
  try {
    ...
    const numberUser = await Utilisateur.findOne({ where: { telephone } });
            if (existingUser) {
              ...
              return res.status(400).json({ status:true,message: 'ok' }); 
            }
  } catch (error) {
...
const registerUser = async (req, res, next) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            // .resize({ width: 500, height: 500 }) // Redimensionnement de l'image
            .toBuffer();
            ...
            // const roleUser = await TypeUtilisateur.findOne({ where: { typeUserId: req.body.typeUserId } });
            // return res.status(403).json({ status: false, message: roleUser.libelle });
            ...
            // }
            // if (newUser && roleUser.libelle === "User") {
              // return res.status(201).json({status:true, message: req.body.typeUserId});
              
                const message = 'L\'utilisateur à bien été enregistré et un e-mail de confiramtion a été envoyé.';
...
//   // Générer un token de réinitialisation de mot de passe
  ...
  res.status(200).json({ status:false,message: 'Un e-mail de réinitialisation de mot de passe a été envoyé.',data:resetToken });
} 
  ...
  Utilisateur.findOne({ where: { resetToken } })
    .then(Utilisateur => {
      if (!Utilisateur) {
        ...
        res.status(401).json({status:false,message:"Token invalide"});
      } else {
        ...
        Otp.findOne({ where: { otp } })
          .then(Otp => {
            if (!Otp) {
              ...
              res.status(401).json({status:false,message:"OTP invalide"});
            } else {
            ...
            // Vérifiez si l'OTP a expiré dans les 2 minutes suivant sa création
              ...
              if (expirationOtp < currentDate) {
                // L'OTP a expiré
                res.status(401).json({status:false,message:"OTP expiré"});
              } else {
                // Les deux vérifications ont réussi, vous pouvez continuer avec votre logique de traitement.
                Otp.destroy();
                res.status(200).json({status:true,message:"Vérification réussie"});
              }
            }
          })
          .catch(error => {
...
const changePassword = async (req,res) => {
  ...
  // console.log(user);
  if (!user) {
    return res.status(404).json({ status:false,message: 'Token non valide.' });
  }
  ...
  res.status(200).json({ status:true,message: 'Votre mot de passe à été réinitialisé.' });
}
...
const validOtpUser = async (req,res) => {
    try {
        ...
        if (!otpRecord || otpRecord.expires_at < new Date()) {
            ...
            res.status(200).json({ status:false,message:'Otp invalides' }); // The OTP is invalid or expired
            
        }
        
        await otpRecord.destroy();
        
        return res.status(200).json({  status:true,message:'Otp valide'}); // The OTP is invalid or expired
...
const generateOtp = async (req,res) => {
    try {
        ...
        return res.status(500).json({ status:false,message: 'Error generating OTP' });
    }
}
...
const deleteUser = async (req,res) => {
  ...
  if (!utilisateur) {
    return res.status(401).json({ status:false,message: 'Cet utilisateur n\'existe pas' });
  }
  utilisateur.isValid = false;
  utilisateur.save();
  res.json({status:true,message: 'l\'utilisateur à été retiré.'});
}
...
const deconnectUser = async (req,res) => {
  ...
  utilisateur.save();
  res.json({status:true,message: 'l\'utilisateur est deconnecté.'});
}
